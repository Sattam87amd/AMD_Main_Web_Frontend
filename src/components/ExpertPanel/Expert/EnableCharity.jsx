  ...
  const fetchCharitySettings = async (expertId) => {
    try {
      ...
      const response = await axios.get(
        `http://amd-api.code4bharat.com/api/expertauth/${expertId}`, // Backend API endpoint with expertId
        {
          headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await axios.put(
        "http://amd-api.code4bharat.com/api/expertauth/update-charity", // Correct backend endpoint
        {
